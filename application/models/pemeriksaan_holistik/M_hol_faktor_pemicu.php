...
class M_hol_faktor_pemicu extends CI_Model
{
  ...
  public function show_modul_by_status_pasien($id_registrasi, $no_bpjs, $column = '*')
  {
    ...
    $this->db->from('hol_faktor_pemicu a');
    $this->db->join('adl_mod_faktor_pemicu b', 'a.id_mod_faktor_pemicu = b.id_mod_faktor_pemicu');
  ...
  {
    $sql = $this->db->set($data)->get_compiled_insert('hol_faktor_pemicu');
    $this->db->query($sql);
  }
  ...
  public function destroy_by_status_pasien($id_registrasi, $no_bpjs)
  {
    $this->db->where('id_registrasi', $id_registrasi);
    $this->db->where('no_bpjs', $no_bpjs);
    $sql = $this->db->set(array('hapus' => '1'))->get_compiled_update('hol_faktor_pemicu');
    $this->db->query($sql);
  }
