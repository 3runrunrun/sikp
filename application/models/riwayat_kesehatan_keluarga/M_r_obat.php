...
class M_r_obat extends CI_Model
{
  ...
  {
    $sql = $this->db->set($data_riwayat)->get_compiled_insert('kk_r_obat');
    $this->db->query($sql);
  }
  ...
  {
    $nilai = array();
    $sql = 'UPDATE kk_r_obat SET jenis_obat = ? WHERE id_kk = ? AND id_konsumsi_obat = ? AND id_riwayat_kes_kel = ? AND no_bpjs = ?';
  ...
  {
    $nilai = array();
    $sql = 'UPDATE kk_r_obat SET hapus = \'1\' WHERE id_kk = ? AND id_konsumsi_obat = ? AND id_riwayat_kes_kel = ? AND no_bpjs = ?';
    echo $sql;
